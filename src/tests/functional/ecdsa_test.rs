...
use crate::ecdsa::{PublicKey, Message, Signature};
...
pub fn run_ecdsa_tests(uart: &mut UartController, verifier: &mut impl EcdsaVerify<PublicKey = PublicKey, Message = Message, Signature = Signature>) {
    for (i, vec) in SECP384R1_TESTVEC.iter().enumerate() {
        let pubkey = PublicKey { qx: vec.qx, qy: vec.qy };
        let sig = Signature { r: vec.r, s: vec.s };
        let msg = Message(vec.m);

        let result = verifier.verify(&pubkey, msg, &sig);
